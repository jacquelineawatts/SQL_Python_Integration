...
def get_project_by_title(title):
    ...
    QUERY = """SELECT description, max_grade FROM projects WHERE title = :title"""
    db_cursor = db.session.execute(QUERY, {'title': title})
    row = db_cursor.fetchone()
    print "{}:{} and the max grade is {}".format(title, row[0], row[1])

def get_grade_by_github_title(github, title):
    """Print grade student received for a project."""

    QUERY = """SELECT project_title, grade FROM grades WHERE student_github = 
            :github AND project_title = :title"""
...
def assign_grade(github, title, grade):
    ...
    db.session.execute(QUERY, {'github': github, 'title': title, 'grade': int(grade)})
    db.session.commit()

    print "You've just assigned the grade {} on {} for {}".format(grade, title, github)
